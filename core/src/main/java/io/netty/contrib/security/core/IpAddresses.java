...
import io.netty.contrib.security.core.comparators.BinarySearchAddressComparator;
...
public class IpAddresses extends SafeListController<Address> implements IpAddressLookup, Comparable<IpAddresses> {
    ...
    public static final IpAddresses ACCEPT_ANY = new IpAddresses(List.of(Address.ANY_ADDRESS)) {
        @Override
    ...
    private IpAddresses(List<Address> addresses) {
        ...
        Objects.requireNonNull(addresses, "List");
    ...
    public static IpAddresses create() {
        return new IpAddresses(List.of());
    }
    ...
    public int lookup(Address address) {
        ...
        return Collections.binarySearch(MAIN_LIST, address, BinarySearchAddressComparator.INSTANCE);
    }
    ...
    public int compareTo(IpAddresses ipAddresses) {
        ...
        if (MAIN_LIST.size() > ipAddresses.MAIN_LIST.size()) {
            return 1;
        } else if (MAIN_LIST.size() < ipAddresses.MAIN_LIST.size()) {
            return -1;
        }
        ...
        for (int i = 0; i < MAIN_LIST.size(); i++) {
            int compare = ipAddresses.MAIN_LIST.get(i).compareTo(this.MAIN_LIST.get(i));
            if (compare != 0) {
                return compare;
            }
        }
    ...
    private static final class SortAndFilterImpl implements SafeListController.SortAndFilter<Address> {
